...
function unindent(
    document: vscode.TextDocument, statement: Statement,
    indentSizePerLevel: number): string {
  let code = '';
  ...
  code += document.getText(new vscode.Range(
      statement.definitionStart.line, indentSizePerLevel,
      statement.definitionEnd.line, statement.definitionEnd.character));
  code += '\n';
...
function prepareExecution(): [
  vscode.TextEditor, vscode.TextDocument, vscode.Position, vscode.Position,
  boolean
] {
  let editor = vscode.window.activeTextEditor;
  if (!editor) {
  ...
  if (document.isUntitled) {
    if (Configuration.executeUntitled) {
      ...
      throw new Error(`Cannot execute code in untitled documents (change the ${
          EXTENSION_PREFIX}.${
          Configuration.executeUntitled} setting to override this behavior)`);
    }
  }
...
function executeCode(code: string, parentClassName?: string, path?: string) {
  _executeCode(
      Terminal.get(), Configuration.reservedKeyword, code,
      Configuration.executeNotImported, parentClassName, path);
}


export function activate(context: vscode.ExtensionContext) {
  let execute =
      vscode.commands.registerCommand(`${EXTENSION_PREFIX}.execute`, () => {
        ...
        try {
          ...
          if (statements.length >= 1 &&
              statements[0].type === StatementType.Class &&
              statements[0].start.line < start.line) {
            let classParent: Statement = statements.shift()!;

            let indentSizePerLevel =
                classParent.children[0].indentSize - classParent.indentSize;

            let code = '';

            for (let child of classParent.children) {
              if ((child.start.line >= start.line &&
                   child.start.line <= end.line) ||
                  (child.end.line >= start.line &&
                   child.end.line <= end.line)) {
                code += unindent(document, child, indentSizePerLevel);
                code += '\n\n';
              }
            }
            ...
            executeCode(
                code, classParent.name,
                (isMain ? undefined : document.fileName));
          }
          ...
          if (statements.length !== 0) {
            ...
            for (let statement of statements) {
              code += document.getText(
                  new vscode.Range(statement.start, statement.end));
              code += '\n\n';
            }

            executeCode(
                code, undefined, (isMain ? undefined : document.fileName));
          }
        } catch (error) {
          console.log(error);
  ...
  let executeAll =
      vscode.commands.registerCommand(`${EXTENSION_PREFIX}.executeAll`, () => {
        let [editor, document, start, end, isMain] = prepareExecution();
        ...
        executeCode(
            document.getText(range), undefined,
            (isMain ? undefined : document.fileName));
      });

  let executeChunck = vscode.commands.registerCommand(
      `${EXTENSION_PREFIX}.executeChunck`, () => {
        let [editor, document, start, end, isMain] = prepareExecution();
  ...
  let dispose =
      vscode.commands.registerCommand(`${EXTENSION_PREFIX}.dispose`, () => {
